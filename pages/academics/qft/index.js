        ...
        <SimpleBar autoHide={true} className='overflow-visible overscroll-y-auto h-full'>
            <div className="h-full translate-y-10 overflow-y-auto overflow-visible overscroll-y-auto pb-28 mb-10 pt-3">
                <div className="max-w-4xl mx-auto p-4 shadow-2xl rounded-lg">
                    <h1 className="text-2xl font-semibold text-accent mb-4 justify-center font-mono text-center">Quantum Field Theory</h1>
                    <div className="overflow-auto">
                        {sections.map((section, index) => (
                            <ExpandableSection key={index} title={`${index + 1}. ${section.title}`} className='flex'>
...
const ExpandableSection = ({ title, children }) => {
    ...
    return (
        <div className="mb-4">
            <div
                className={`flex items-center cursor-pointer ${expanded ? 'mb-2' : ''}`}
